...
for collection_name in collections:
    ...
    # Load documents with isDelete = "false"
    documents = []
    query = {
        "isDelete": "false",
        **{field_name: {"$exists": True} for field_name in field_names}
    }
    ...
    if not documents:
        print(f"⚠️ No valid data found in {collection_name} with isDelete='false'")
        continue
